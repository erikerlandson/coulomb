...
object accepted:
    ...
    final type Percent
    given unit_Percent: DerivedUnit[Percent, 1 / 100, "percent", "%"] =
        DerivedUnit()
    ...
    given unit_Milliliter
        : DerivedUnit[Milliliter, Liter / 1000, "milliliter", "ml"] =
        DerivedUnit()

    final type Gram
    given unit_Gram: DerivedUnit[Gram, Kilogram / 1000, "gram", "g"] =
        DerivedUnit()

    final type Tonne
    given unit_Tonne: DerivedUnit[Tonne, 1000 * Kilogram, "tonne", "t"] =
        DerivedUnit()

    final type Millibar
    given unit_Millibar: DerivedUnit[
        Millibar,
