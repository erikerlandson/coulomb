...
class CoefficientSuite extends CoulombSuite:
    ...
    test("convertible units") {
        ...
        assertEquals(coefficient[Rational, Kilogram * Meter / (Second ^ 2), Pound * Yard / (Minute ^ 2)],
                     Rational(50000000000000L, 5760623099L))
        assertEquals(coefficient[Rational, Meter ^ 3, 1000 * Liter], Rational(1))
        assertEquals(coefficient[Rational, Meter, Kilo * Yard], Rational(10, 9144))
    }

    test("non-convertible units") {
        assertCE("coefficient[Rational, Meter, Second]")
        assertCE("coefficient[Rational, Meter ^ 2, Liter]")
        assertCE("coefficient[Rational, Kilogram * Meter / (Second ^ 2), Pound * Yard / (Minute ^ 3)]")
    }
    ...
    test("units with embedded coefficients") {
        ...
        assertEquals(coefficient[Rational, (10 ^ 6) * Meter, Meter], Rational(1000000))
        assertEquals(coefficient[Rational, Meter,  1.5 * Meter], Rational(2, 3))
        assertEquals(coefficient[Rational, Meter,  1.25f * Meter], Rational(4, 5))
        assertEquals(coefficient[Rational, ((1 / 3L) * (10L ^ 100)) * Meter, Meter], Rational(1, 3) * Rational(10).pow(100))
    }
