...
class QuantityParserTests extends munit.FunSuite {
  import coulomb.parser.QuantityParser
  import ConfigIntegration._

  test("return configured unit quantities") {
    assert(conf.getQuantity[Double, Second]("duration").get.isValidQ[Double, Second](60))
    assert(conf.getQuantity[Int, Giga %* Byte]("memory").get.isValidQ[Int, Giga %* Byte](100))
    assert(conf.getQuantity[Float, Mega %* Byte %/ Second]("bandwidth").get.isValidQ[Float, Mega %* Byte %/ Second](10))
  }

  test("return convertable unit quantities") {
    assert(conf.getQuantity[Double, Minute]("duration").get.isValidQ[Double, Minute](1))
    assert(conf.getQuantity[Int, Giga %* Bit]("memory").get.isValidQ[Int, Giga %* Bit](800))
    assert(conf.getQuantity[Float, Giga %* Bit %/ Minute]("bandwidth").get.isValidQ[Float, Giga %* Bit %/ Minute](4.8))
  }
