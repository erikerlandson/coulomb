...
package coulomb.integrations.pureconfig.io
...
object rational:
    ...
    given g_RationalReader: ConfigReader[Rational] =
    ...
    given g_RationalWriter: ConfigWriter[Rational] =
        ConfigWriter.fromFunction[Rational] { r =>
            if (r.denominator == 1)
                ...
                ConfigValueFactory.fromAnyRef(
                    Map(
                        "n" -> r.numerator.toCV,
                        "d" -> r.denominator.toCV
                    ).asJava
                )
        }

object ruDSL:
    given g_RuntimeUnit_DSL_Reader(using
    ...
    given g_RuntimeUnit_DSL_Writer(using
        parser: RuntimeUnitParser
...
object ruJSON:
    import coulomb.integrations.pureconfig.{UnitPathMapper, PathUnitMapper}

    given g_RuntimeUnit_JSON_Reader(using
    ...
    given g_RuntimeUnit_JSON_Writer(using
...
object runtimeq:
    given g_RuntimeQuantity_Reader[V](using
        ConfigReader[V],
        ConfigReader[RuntimeUnit]
    ...
    given g_RuntimeQuantity_Writer[V](using
        ConfigWriter[V],
        ConfigWriter[RuntimeUnit]
...
object quantity:
    ...
    inline given g_Quantity_Reader_VC[V, U](using
        fv: Fractional[V],
    ...
    inline given g_Quantity_Reader_NoVC[V, U](using
        nocv: NotGiven[
            GivenAll[
                (
                    Fractional[V],
    ...
    inline given g_Quantity_Writer[V, U](using
        ConfigWriter[RuntimeQuantity[V]]
