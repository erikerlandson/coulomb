...
object value:
    ...
    given ctx_ConvertableToCoulombRational: ConvertableTo[CoulombRational]
        ...
        def fromByte(a: Byte): CoulombRational =
            CoulombRational(BigInt(a), bigint1)
        def fromShort(a: Short): CoulombRational =
            CoulombRational(BigInt(a), bigint1)
        def fromInt(a: Int): CoulombRational =
            CoulombRational(BigInt(a), bigint1)
        def fromLong(a: Long): CoulombRational =
            CoulombRational(BigInt(a), bigint1)
        def fromFloat(a: Float): CoulombRational = fromRational(
            SpireRational(a)
        )
        def fromDouble(a: Double): CoulombRational = fromRational(
            SpireRational(a)
        )
        def fromBigInt(a: BigInt): CoulombRational = CoulombRational(a, bigint1)
        def fromBigDecimal(a: BigDecimal): CoulombRational = fromRational(
            SpireRational(a)
        )
        ...
        def fromAlgebraic(a: Algebraic): CoulombRational =
            fromRational(
                a.toRational.getOrElse(
                    SpireRational(a.toBigDecimal(MathContext.DECIMAL64))
                )
            )
    ...
    given ctx_ConvertableFromCoulombRational: ConvertableFrom[CoulombRational]
        ...
        def toBigDecimal(a: CoulombRational): BigDecimal =
            toRational(a).toBigDecimal(MathContext.DECIMAL64)
        def toRational(a: CoulombRational): SpireRational =
            SpireRational(a.n, a.d)
        def toAlgebraic(a: CoulombRational): Algebraic = Algebraic(
            toRational(a)
        )
        def toReal(a: CoulombRational): Real = Real(toRational(a))
        def toNumber(a: CoulombRational): Number = Number(toRational(a))
        def toType[B: ConvertableTo](a: CoulombRational): B =
            ConvertableTo[B].fromRational(toRational(a))
