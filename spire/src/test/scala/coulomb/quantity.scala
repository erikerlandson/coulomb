...
class SpireQuantitySuite extends CoulombSuite:
    ...
    test("addition standard") {
        ...
        (1.withUnit[Meter] + BigInt(1).withUnit[Meter])
            .assertQ[BigInt, Meter](BigInt(2))
        (BigInt(1).withUnit[Meter] + 1.withUnit[Meter])
            .assertQ[BigInt, Meter](BigInt(2))
        (1f.withUnit[Meter] + BigDecimal(1).withUnit[Meter])
            .assertQ[BigDecimal, Meter](BigDecimal(2))
        (BigDecimal(1).withUnit[Meter] + 1f.withUnit[Meter])
            .assertQ[BigDecimal, Meter](BigDecimal(2))
        (1.withUnit[Meter] + Real(1).withUnit[Meter])
            .assertQ[Real, Meter](Real(2))
        (Real(1).withUnit[Meter] + 1.withUnit[Meter])
            .assertQ[Real, Meter](Real(2))

        (Rational(1).withUnit[Meter] + BigDecimal(1).withUnit[Yard])
            .assertQ[Rational, Meter](Rational(19144, 10000))
        (BigDecimal(1).withUnit[Meter] + Rational(1).withUnit[Yard])
            .assertQ[Rational, Meter](Rational(19144, 10000))
        ...
        (BigInt(1).withUnit[Meter] + BigInt(1).withUnit[Yard].tToUnit[Meter])
            .assertQ[BigInt, Meter](BigInt(1))
    }
    ...
    test("tpow") {
        ...
        BigInt(10)
            .withUnit[Meter]
            .tpow[2]
            .assertQ[BigInt, Meter ^ 2](BigInt(100))
        BigInt(10)
            .withUnit[Meter]
            .tpow[-1]
            .assertQ[BigInt, 1 / Meter](BigInt(0))
        BigInt(10)
            .withUnit[Meter]
            .tpow[1 / 2]
            .assertQ[BigInt, Meter ^ (1 / 2)](BigInt(3))
        BigInt(10)
            .withUnit[Meter]
            .tpow[-1 / 2]
            .assertQ[BigInt, 1 / (Meter ^ (1 / 2))](BigInt(0))
    }
    ...
    test("< standard") {
        ...
        assertEquals(
            Rational(1).withUnit[Yard] < BigInt(1).withUnit[Meter],
            true
        )
        assertEquals(
            BigDecimal(1).withUnit[Meter] < Rational(1).withUnit[Yard],
        ...
        assertEquals(
            Algebraic(1).withUnit[Yard] < Real(1).withUnit[Meter],
            true
        )
        assertEquals(
            Rational(1).withUnit[Meter] < BigDecimal(1).withUnit[Yard],
